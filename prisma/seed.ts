...
async function main() {
	...
	// Create a set of unique account names
	const accountNames = new Set(
		data.flatMap((t) => [t.fromAccount, t.toAccount])
	);

	// Create an account for each unique name
	for (let name of accountNames) {
		await prisma.account.upsert({
			where: { accountName: name },
			update: {},
			create: { accountName: name },
		});
	}

	// Create the transactions
	for (let transaction of data) {
		// Fetch the ids of the fromAccount and toAccount
		const fromAccount = await prisma.account.findUnique({
			where: { accountName: transaction.fromAccount },
		});
		const toAccount = await prisma.account.findUnique({
			where: { accountName: transaction.toAccount },
		});
